@mod @mod_opendsa
Feature: A teacher can choose one of 4 options for publishing opendsa results
  In order to display opendsa activities outcomes
  As a teacher
  I need to publish the opendsa activity results in different ways
  ...
  Scenario: Do not publish results to students
    ...
    When I choose "Option 1" from "OpenDSA 1" opendsa activity
  ...
  Scenario: Show results to students after they answer
    ...
    And I am on "Course 1" course homepage
    And I choose "Option 1" from "OpenDSA 1" opendsa activity
    And I should see "Your selection: Option 1"
    And I should see "Responses"

  Scenario: Show results to students only after the opendsa is closed
    Given I add a "OpenDSA" to section "1" and I fill the form with:
      | OpenDSA name | OpenDSA 1 |
      | Description | OpenDSA Description |
