@mod @mod_opendsa
Feature: Multiple option opendsa response
  In order to ask questions as a opendsa of multiple responses
  As a teacher
  I need to add opendsa activities to courses with multiple options enabled

  Scenario: Complete a opendsa with multiple options enabled
    ...
    And I add a "OpenDSA" to section "1" and I fill the form with:
      ...
      | Allow more than one opendsa to be selected | Yes |
      | option[0] | Option 1 |
      | option[1] | Option 2 |
    And I log out
  ...
  Scenario: Complete a opendsa with multiple options enabled and limited responses set
    Given the following "users" exist:
      | username | firstname | lastname | email |
      | teacher1 | Teacher | 1 | teacher1@example.com |
      | student1 | Student | 1 | student1@example.com |
    ...
    And the "#opendsa_1" "css_element" should be disabled
    And the "#opendsa_2" "css_element" should be disabled
    And the "#opendsa_3" "css_element" should be enabled
