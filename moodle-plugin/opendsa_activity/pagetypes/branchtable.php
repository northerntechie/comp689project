...
class lesson_page_type_branchtable extends lesson_page {
    ...
    public static function get_jumptooptions($firstpage, lesson $lesson) {
        ...
        if (!$firstpage) {
            if (!$apageid = $DB->get_field("lesson_pages", "id", array("opendsa_activity_id" => $lesson->id, "prevpageid" => 0))) {
    ...
    public function check_answer() {
        ...
        if ($grades = $DB->get_records("lesson_grades", array("opendsa_activity_id" => $this->lesson->id, "userid" => $USER->id), "grade DESC")) {
        ...
        $branch = new stdClass;
        $branch->opendsa_activity_id = $this->lesson->id;
